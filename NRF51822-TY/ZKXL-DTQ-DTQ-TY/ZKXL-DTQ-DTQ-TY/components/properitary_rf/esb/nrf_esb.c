...
nrf_esb_config_t             m_config_local;		//本来为内部静态变量，改为外部变量
...
//	printf(" NRF_RADIO->INTENCLR %04X ", NRF_RADIO->INTENCLR);
    NRF_RADIO->INTENCLR = 0xFFFFFFFF;
//	printf(" %04X \r\n", NRF_RADIO->INTENCLR);
	
//	printf(" NRF_RADIO->EVENTS_DISABLED  %04X ", NRF_RADIO->EVENTS_DISABLED );
    NRF_RADIO->EVENTS_DISABLED = 0;
...
//	printf(" NRF_RADIO->SHORTS  %04X ", NRF_RADIO->SHORTS );
    NRF_RADIO->SHORTS      = RADIO_SHORTS_COMMON | RADIO_SHORTS_DISABLED_TXEN_Msk;
//	printf(" %04X \r\n",NRF_RADIO->SHORTS );
    
//	printf(" NRF_RADIO->INTENSET  %04X ", NRF_RADIO->INTENSET );
    NRF_RADIO->INTENSET    = RADIO_INTENSET_DISABLED_Msk;
//	printf(" %04X \r\n",NRF_RADIO->INTENSET );
...
//	printf(" NRF_RADIO->RXADDRESSES  %04X ", NRF_RADIO->RXADDRESSES );
    NRF_RADIO->RXADDRESSES  = m_esb_addr.rx_pipes_enabled;
//	printf(" %04X \r\n",NRF_RADIO->RXADDRESSES );
	
//	printf(" NRF_RADIO->FREQUENCY  %04X ", NRF_RADIO->FREQUENCY );
    NRF_RADIO->FREQUENCY    = m_esb_addr.rf_channel;
//	printf(" %04X \r\n",NRF_RADIO->FREQUENCY );
    
//	printf(" NRF_RADIO->PACKETPTR  %04X ", NRF_RADIO->PACKETPTR );
    NRF_RADIO->PACKETPTR    = (uint32_t)m_rx_payload_buffer;
//	printf(" %04X \r\n",NRF_RADIO->PACKETPTR );
...
//用户自定义函数
void my_esb_mode_change(nrf_esb_mode_t mode)
{
	ret_code_t err_code;
	
	if(mode == NRF_ESB_MODE_PRX)
	{
		if(NRF_ESB_MODE_PRX != m_config_local.mode)
		{
			//干掉：可能处于发送状态，但是没有发送成功，就不会回到空闲状态，导致程序卡死在这里
//			while(NRF_ESB_STATE_IDLE != m_nrf_esb_mainstate)
//			{
//				;//等待系统回到空闲状态（若正在发送数据，发送完成后自动回到空闲状态）
//			}
			m_config_local.mode = NRF_ESB_MODE_PRX;
			err_code = nrf_esb_set_rf_channel(4);		//只有空闲状态才能重新配置频点、通道、地址等参数
		}
	}
	else if(mode == NRF_ESB_MODE_PTX)
	{
		if((NRF_ESB_MODE_PTX != m_config_local.mode))
		{
