...
static	void LCD_write_data(uint8_t data, uint8_t count, uint8_t flag);								   
static	void LCD_write_command(uint8_t cmd);
static	void LCD_write_signle_data(uint8_t addr, uint8_t data);
static	void LCD_write_mult_data(uint8_t addr, uint8_t len, uint8_t * data);
static	void clear_ram(uint8_t ram1, uint8_t ram2);
static	void modify_ram(uint8_t ram1, uint8_t ram2, uint8_t data1, uint8_t data2);
static	void modify_segment(uint8_t ram1, uint8_t ram2, uint8_t data);
static	void LCD_diplay_on(void);							   
static	void LCD_diplay_off(void);									   
static	void LCD_display_sleep(void);							   
static	void LCD_display_wakeup(void);								   
static	void LCD_clear_screen(void);								   
static	void LCD_clear_letter(void);
static	void LCD_diplay_letter(uint8_t data);
static	void LCD_display_true_false(uint8_t tmp);
static	void LCD_display_ok_failure(uint8_t tmp);
static	void LCD_diplay_signal(uint8_t tmp)	;
static	void LCD_diplay_battery_level(uint8_t tmp);
//static	void LCD_clear_segment(void);
//static	void LCD_diplay_segment(uint8_t num, uint8_t data);
static	void LCD_display_update( void );
static void LCD_gpio_config(bool tmp);
...
static	void LCD_diplay_on(void)
{
//	uint8_t tmp_ram[18];				//这样显示浪费功耗，去掉
//	
...
static	void LCD_diplay_signal(uint8_t tmp)					//显示信号强度
{	
...
static	void LCD_diplay_battery_level(uint8_t tmp)			//显示电池电量
{
...
void LCD_init(void)
{
	...
	LCD->update       = LCD_display_update;
	
	LCD->on 	      = LCD_diplay_on;
	LCD->off 	      = LCD_diplay_off;
	LCD->sleep    	  = LCD_display_sleep;
	LCD->wakeup       = LCD_display_wakeup;
	
	LCD->timer_start  = TIMER->lcd_update_start;		//指向定时器的函数指针
	LCD->timer_stop   = TIMER->lcd_update_stop;
	LCD->timer_handler= TIMER->lcd_update_handler;
	
	LCD->battery      = LCD_diplay_battery_level;
	LCD->send_result  = LCD_display_ok_failure;
	LCD->judge        = LCD_display_true_false;
	LCD->signal       = LCD_diplay_signal;
	LCD->choice       = LCD_diplay_letter;
	LCD->student_id   = LCD_display_student_id;
	LCD->question_num = LCD_display_question_num;
	LCD->grade_value  = LCD_display_grade_value;
	LCD->clear_segment= LCD_clear_segment; 
}
