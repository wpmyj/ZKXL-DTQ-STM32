...
{	
	...
	APP->command_handler = command_handler;
...
//	app_debug("KEY->value %d \r\n",KEY->value);
	...
	if(KEY->press_flg)				
	{
		...
		TIMER->powerdown_stop();		
		TIMER->powerdown_start();		//有按键按下时重新开启系统待机定时器
...
void key_letter_input_handler(void)
{
	...
	if(ANSWER.type[ANSWER.current_num] != TRUE_OR_FALSE)
	{
		LCD->choice(ANSWER.answer[ANSWER.current_num].choice_answer);
	}
}
...
{		
	if(ANSWER.grade_or_score == SCORE_RESULT)					//互动投票模式
	{
		...
		LCD->grade_value(ANSWER.score);							//显示评分
	}
	else if(ANSWER.grade_or_score == GRADE_RESULT) 				//小组评分模式
	{
		 if(6 == KEY->value)							//小组评分范围1-5
			 return;
		ANSWER.grade[ANSWER.current_num] = KEY->value;
		LCD->grade_value(ANSWER.grade[ANSWER.current_num]);			//投票结果
	}
}
...
void key_judge_input_handler(void)
{
	if(ANSWER.total_num == 0)
	{
		...
		LCD->judge(ANSWER.answer[ANSWER.current_num].true_or_false);		//LCD显示
	}
...
void key_clear_handler(void)
{
	...
	//过关斩将后，显示题号
	LCD->question_num(ANSWER.number[ANSWER.current_num]);
}
...
//		CLOCK->HFCLK_start();   		//发送前，打开外部时钟， 否则radio外设不可用
		nrf_start_transmit();		
	}
}
...
void key_last_question_handler(void)
{
	...
	if(ANSWER.score_or_answer == ANSWER_INPUT)								//如果输入模式为作答
	{
		...
		switch(ANSWER.type[ANSWER.current_num])
		{
			...
			case TRUE_OR_FALSE:
				LCD->choice(0x00);
				LCD->judge(ANSWER.answer[ANSWER.current_num].choice_answer);	
				LCD->question_num(ANSWER.number[ANSWER.current_num]);	
			default :
				break;
		}
	...
	else if(ANSWER.score_or_answer == SCORE_INPUT)			
	{
		...
		else if(ANSWER.grade_or_score == GRADE_RESULT) //小组评分模式
		{
			...
			LCD->question_num(ANSWER.number[ANSWER.current_num]);
			LCD->grade_value(ANSWER.grade[ANSWER.current_num]);
		}
	}
}
...
void multiple_question_command_handler(void)
{
	...
	LCD->choice(0x00);														//清除字母显示
	LCD->judge(0x00);														//清除对错显示
	LCD->clear_segment();													//清除数码管显示
	LCD->student_id(STUDENT_ID);
	
	LCD->question_num(ANSWER.number[ANSWER.current_num]);			//显示当前作答题号
}
...
void group_grade_command_handler(void)
{
	...
	LCD->grade_value(ANSWER.grade[ANSWER.current_num]);					//显示当前评分结果
	LCD->question_num(ANSWER.number[ANSWER.current_num]);			//显示当前评分组号
}
...
void activity_end_command_handler(void)
{
	...
	LCD->judge(0x00);										//清除对错显示
	LCD->clear_segment();									//清除数码管显示
	LCD->student_id(STUDENT_ID);								//学号不清空
}
