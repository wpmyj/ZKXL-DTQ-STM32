...
{
	RADIO->rx_window_on_flg = false;
	TIMER_RxWindowStart();		//接收时间窗定时器打开
}

void RADIO_Sleep(void)
{
	TIMER_RxWindowStop();	//接收时间窗定时器停止
...
//	tmp_debug("\r\n");
	...
	if (false == TRANSPORT.tx_ing_flag)
	{
		...
		TIMER_RetransmitStart();				//启动重发定时器
	}
}

/******************************************************************************
  @函数:RADIO_RxDataHandler
...
//							LCD_display_student_id(rx_payload.rssi & 0x7f);
						...
						case NRF_DATA_IS_PRE:
							if(0x00 == last_rx_pack_num)	//包号开机初始化为0，开机时无条件打开RX窗接收数据
							{
								if(false == RADIO->rx_window_wait_data)
								{
									...
									{
										TIMER_WaitDataStart(110 - rx_payload.data[9] - 5);	//留5个前导帧的余量
									}
									else
									{
