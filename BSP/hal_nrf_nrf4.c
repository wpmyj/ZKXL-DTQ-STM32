...
{
	config_t config;
	config.value = hal_nrf_read_reg_4(CONFIG);
	...
	hal_nrf_write_reg_4(CONFIG, config.value);
}
...
uint8_t hal_nrf_get_clear_irq_flags_4(void)
{
	...
	retval = hal_nrf_write_reg_4(STATUS_2, (BIT_6|BIT_5|BIT_4));
...
{
	hal_nrf_write_reg_4(STATUS_2, _SET_BIT(int_source));
}
...
{
	return hal_nrf_nop_4() & (BIT_6|BIT_5|BIT_4);
}
...
void hal_nrf_open_pipe_4(hal_nrf_address_t pipe_num, bool auto_ack)
{
	...
	en_pipes_t en_aa;
	en_rxaddr.value = hal_nrf_read_reg_4(EN_RXADDR);
	en_aa.value = hal_nrf_read_reg_4(EN_AA);
	...
	hal_nrf_write_reg_4(EN_RXADDR, en_rxaddr.value);
	hal_nrf_write_reg_4(EN_AA, en_aa.value);
}
...
void hal_nrf_set_address_4(const hal_nrf_address_t address, const uint8_t *addr)
{
	...
	case HAL_NRF_PIPE0:
	case HAL_NRF_PIPE1:
		hal_nrf_write_multibyte_reg_4(W_REGISTER + RX_ADDR_P0 + (uint8_t) address, addr, hal_nrf_get_address_width_4());
		break;
	case HAL_NRF_PIPE2:
	case HAL_NRF_PIPE3:
	case HAL_NRF_PIPE4:
	case HAL_NRF_PIPE5:
		hal_nrf_write_reg_4(RX_ADDR_P0 + (uint8_t) address, *addr);
		break;
...
uint8_t hal_nrf_get_address_4(uint8_t address, uint8_t *addr)
{
	...
	case HAL_NRF_PIPE1:
	case HAL_NRF_TX:
		return hal_nrf_read_multibyte_reg_4(address, addr);
	default:
		*addr = hal_nrf_read_reg_4(RX_ADDR_P0 + address);
		return 1;
	}
}
...
void hal_nrf_set_auto_retr_4(uint8_t retr, uint16_t delay)
{
	...
	hal_nrf_write_reg_4(SETUP_RETR, setup_retr.value);
}
...
/*******************************************************************************
  ...
  * @retval None
*******************************************************************************/
void hal_nrf_set_address_width_4(hal_nrf_address_width_t aw)
{
	setup_aw_t setup_aw;
	setup_aw.bits.aw = (uint8_t)aw - 2;

	hal_nrf_write_reg_4(SETUP_AW, setup_aw.value);
}
...
{
	return hal_nrf_read_reg_4(SETUP_AW) + 2;
}
...
{
	hal_nrf_write_reg_4(RX_PW_P0 + pipe_num, pload_width);
}
...
{
	return hal_nrf_read_reg_4(OBSERVE_TX);
}
...
{
	return ((hal_nrf_read_reg_4(OBSERVE_TX) & (BIT_7|BIT_6|BIT_5|BIT_4)) >> 4);
}
...
uint8_t hal_nrf_get_rx_payload_width_4(uint8_t pipe_num)
{
	...
	{
	case 0:
		pw = hal_nrf_read_reg_4(RX_PW_P0);
		break;
	case 1:
		pw = hal_nrf_read_reg_4(RX_PW_P1);
		break;
	case 2:
		pw = hal_nrf_read_reg_4(RX_PW_P2);
		break;
	case 3:
		pw = hal_nrf_read_reg_4(RX_PW_P3);
		break;
	case 4:
		pw = hal_nrf_read_reg_4(RX_PW_P4);
		break;
	case 5:
		pw = hal_nrf_read_reg_4(RX_PW_P5);
		break;
	default:
		pw = 0;
		break;
...
void hal_nrf_set_rf_channel_4(uint8_t channel)
{
	...
	hal_nrf_write_reg_4(RF_CH, rf_ch.value);
}
...
{
	rf_setup_t rf_setup;
	rf_setup.value = hal_nrf_read_reg_4(RF_SETUP);

	rf_setup.bits.rf_pwr = (uint8_t)power;

	hal_nrf_write_reg_4(RF_SETUP, rf_setup.value);
}
...
{
	if (hal_nrf_get_rx_data_source_4()==7)
	{
		return TRUE;
	}
	else
...
{
	return (bool)((hal_nrf_read_reg_4(FIFO_STATUS)>> RX_FULL) & 1);
}
...
{
	return (bool)((hal_nrf_read_reg_4(FIFO_STATUS) >> TX_EMPTY) & 1);
}
...
{
	return (bool)((hal_nrf_read_reg_4(FIFO_STATUS) >> TX_FIFO_FULL) & 1);
}
...
{
	return ((hal_nrf_read_reg_4(FIFO_STATUS) & ((1<<TX_FIFO_FULL)|(1<<TX_EMPTY))) >> 4);
}
...
{
	return (hal_nrf_read_reg_4(FIFO_STATUS) & ((1<<RX_FULL)|(1<<RX_EMPTY)));
}
...
{
	return hal_nrf_read_reg_4(FIFO_STATUS);
}
...
{
	return (hal_nrf_read_reg_4(OBSERVE_TX) & (BIT_3|BIT_2|BIT_1|BIT_0));
}
...
{
	return (hal_nrf_read_reg_4(CD) & 0x01);
}
...
void hal_nrf_setup_dynamic_payload_4(uint8_t setup)
{
	...
	hal_nrf_write_reg_4(DYNPD_2, dynpd.value);
}
...
{
	feature_t feature;
	feature.value = hal_nrf_read_reg_4(FEATURE_2);
	feature.bits.en_dpl = (enable) ? 1 : 0;

	hal_nrf_write_reg_4(FEATURE_2, feature.value);
}
...
{
	hal_nrf_write_multibyte_reg_4(W_TX_PAYLOAD, tx_pload, length);
}
...
{
	hal_nrf_write_multibyte_reg_4(W_TX_PAYLOAD_NOACK, tx_pload, length);
}
...
{
	hal_nrf_write_multibyte_reg_4(W_ACK_PAYLOAD | pipe, tx_pload, length);
}
...
{
	return hal_nrf_read_reg_4(R_RX_PL_WID);
}
...
{
	return hal_nrf_read_multibyte_reg_4((uint8_t)(HAL_NRF_RX_PLOAD), rx_pload);
}
...
{
	return ((hal_nrf_nop_4() & (BIT_3|BIT_2|BIT_1)) >> 1);
}
...
{
	return (bool)((hal_nrf_get_fifo_status_4() & (1<<TX_REUSE)) >> TX_REUSE);
}
...
uint8_t hal_nrf_write_reg_4(uint8_t reg, uint8_t value)
{
	...
	CSN_LOW_4();
	retval = hal_nrf_rw_4(W_REGISTER + reg);
	hal_nrf_rw_4(value);
	CSN_HIGH_4();
...
uint16_t hal_nrf_read_multibyte_reg_4(uint8_t reg, uint8_t *pbuf)
{
	...
	case HAL_NRF_PIPE1:
	case HAL_NRF_TX:
			length = ctr = hal_nrf_get_address_width_4();
			CSN_LOW_4();
			hal_nrf_rw_4(RX_ADDR_P0 + reg);
			break;

	case HAL_NRF_RX_PLOAD:
			if ( (reg = hal_nrf_get_rx_data_source_4()) < 7)
			{
				length = ctr = hal_nrf_read_rx_payload_width_4();

				CSN_LOW_4();
				hal_nrf_rw_4(R_RX_PAYLOAD);
			}
			else
			{
				ctr = length = 0;
...
/*******************************************************************************
  ...
  * @retval None
*******************************************************************************/
void hal_nrf_write_multibyte_reg_4(uint8_t cmd, const uint8_t *pbuf, uint8_t length)
{
	CSN_LOW_4();
	hal_nrf_rw_4(cmd);
	while(length--)
...
*******************************************************************************/
uint8_t hal_nrf_rw_4(uint8_t uWrittenByte)
{
	uint8_t i = 0;
	...
	for(i = 0; i < 8; i++)
	{
		...
		CLK_HIGH_4();
		// if(MISO_INPUT_4())
		if((GPIOB->IDR & GPIO_Pin_10) != (uint32_t)Bit_RESET)
		{
			uTmp |= 0x01;
		}
		else
