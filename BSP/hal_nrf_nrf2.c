...
{
	config_t config;
	config.value = hal_nrf_read_reg_2 (CONFIG);
	...
	hal_nrf_write_reg_2 (CONFIG, config.value);
}
...
uint8_t hal_nrf_get_clear_irq_flags_2(void)
{
	...
	retval = hal_nrf_write_reg_2 (STATUS_2, (BIT_6|BIT_5|BIT_4));
...
{
	hal_nrf_write_reg_2 (STATUS_2, _SET_BIT(int_source));
}
...
{
	return hal_nrf_nop_2() & (BIT_6|BIT_5|BIT_4);
}
...
void hal_nrf_open_pipe_2(hal_nrf_address_t pipe_num, bool auto_ack)
{
	...
	en_pipes_t en_aa;
	en_rxaddr.value = hal_nrf_read_reg_2 (EN_RXADDR);
	en_aa.value = hal_nrf_read_reg_2 (EN_AA);
	...
	hal_nrf_write_reg_2 (EN_RXADDR, en_rxaddr.value);
	hal_nrf_write_reg_2 (EN_AA, en_aa.value);
}
...
void hal_nrf_set_address_2(const hal_nrf_address_t address, const uint8_t *addr)
{
	...
	case HAL_NRF_PIPE0:
	case HAL_NRF_PIPE1:
		hal_nrf_write_multibyte_reg_2(W_REGISTER + RX_ADDR_P0 + (uint8_t) address, addr, hal_nrf_get_address_width_2());
		break;
	case HAL_NRF_PIPE2:
	case HAL_NRF_PIPE3:
	case HAL_NRF_PIPE4:
	case HAL_NRF_PIPE5:
		hal_nrf_write_reg_2 (RX_ADDR_P0 + (uint8_t) address, *addr);
		break;
...
uint8_t hal_nrf_get_address_2(uint8_t address, uint8_t *addr)
{
	...
	case HAL_NRF_PIPE1:
	case HAL_NRF_TX:
		return hal_nrf_read_multibyte_reg_2 (address, addr);
	default:
		*addr = hal_nrf_read_reg_2(RX_ADDR_P0 + address);
		return 1;
	}
}
...
void hal_nrf_set_auto_retr_2(uint8_t retr, uint16_t delay)
{
	...
	hal_nrf_write_reg_2 (SETUP_RETR, setup_retr.value);
}
...
/*******************************************************************************
  ...
  * @retval None
*******************************************************************************/
void hal_nrf_set_address_width_2(hal_nrf_address_width_t aw)
{
	setup_aw_t setup_aw;
	setup_aw.bits.aw = (uint8_t)aw - 2;

	hal_nrf_write_reg_2 (SETUP_AW, setup_aw.value);
}
...
{
	return hal_nrf_read_reg_2 (SETUP_AW) + 2;
}
...
{
	hal_nrf_write_reg_2 (RX_PW_P0 + pipe_num, pload_width);
}
...
{
	return hal_nrf_read_reg_2(OBSERVE_TX);
}
...
{
	return ((hal_nrf_read_reg_2(OBSERVE_TX) & (BIT_7|BIT_6|BIT_5|BIT_4)) >> 4);
}
...
uint8_t hal_nrf_get_rx_payload_width_2(uint8_t pipe_num)
{
	...
	{
	case 0:
		pw = hal_nrf_read_reg_2 (RX_PW_P0);
		break;
	case 1:
		pw = hal_nrf_read_reg_2 (RX_PW_P1);
		break;
	case 2:
		pw = hal_nrf_read_reg_2 (RX_PW_P2);
		break;
	case 3:
		pw = hal_nrf_read_reg_2 (RX_PW_P3);
		break;
	case 4:
		pw = hal_nrf_read_reg_2 (RX_PW_P4);
		break;
	case 5:
		pw = hal_nrf_read_reg_2 (RX_PW_P5);
		break;
	default:
		pw = 0;
		break;
...
void hal_nrf_set_rf_channel_2(uint8_t channel)
{
	...
	hal_nrf_write_reg_2 (RF_CH, rf_ch.value);
}
...
{
	rf_setup_t rf_setup;
	rf_setup.value = hal_nrf_read_reg_2 (RF_SETUP);

	rf_setup.bits.rf_pwr = (uint8_t)power;

	hal_nrf_write_reg_2 (RF_SETUP, rf_setup.value);
}
...
{
	if (hal_nrf_get_rx_data_source_2()==7)
	{
		return TRUE;
	}
	else
...
{
	return (bool)((hal_nrf_read_reg_2(FIFO_STATUS)>> RX_FULL) & 1);
}
...
{
	return (bool)((hal_nrf_read_reg_2(FIFO_STATUS) >> TX_EMPTY) & 1);
}
...
{
	return (bool)((hal_nrf_read_reg_2(FIFO_STATUS) >> TX_FIFO_FULL) & 1);
}
...
{
	return ((hal_nrf_read_reg_2(FIFO_STATUS) & ((1<<TX_FIFO_FULL)|(1<<TX_EMPTY))) >> 4);
}
...
{
	return (hal_nrf_read_reg_2(FIFO_STATUS) & ((1<<RX_FULL)|(1<<RX_EMPTY)));
}
...
{
	return hal_nrf_read_reg_2(FIFO_STATUS);
}
...
{
	return (hal_nrf_read_reg_2(OBSERVE_TX) & (BIT_3|BIT_2|BIT_1|BIT_0));
}
...
{
	return (hal_nrf_read_reg_2(CD) & 0x01);
}
...
void hal_nrf_setup_dynamic_payload_2(uint8_t setup)
{
	...
	hal_nrf_write_reg_2 (DYNPD_2, dynpd.value);
}
...
{
	feature_t feature;
	feature.value = hal_nrf_read_reg_2 (FEATURE_2);
	feature.bits.en_dpl = (enable) ? 1 : 0;

	hal_nrf_write_reg_2 (FEATURE_2, feature.value);
}
...
{
	hal_nrf_write_multibyte_reg_2(W_TX_PAYLOAD, tx_pload, length);
}
...
{
	hal_nrf_write_multibyte_reg_2(W_TX_PAYLOAD_NOACK, tx_pload, length);
}
...
{
	hal_nrf_write_multibyte_reg_2(W_ACK_PAYLOAD | pipe, tx_pload, length);
}
...
{
	return hal_nrf_read_reg_2(R_RX_PL_WID);
}
...
{
	return hal_nrf_read_multibyte_reg_2((uint8_t)(HAL_NRF_RX_PLOAD), rx_pload);
}
...
{
	return ((hal_nrf_nop_2() & (BIT_3|BIT_2|BIT_1)) >> 1);
}
...
{
	return (bool)((hal_nrf_get_fifo_status_2() & (1<<TX_REUSE)) >> TX_REUSE);
}
...
uint8_t hal_nrf_write_reg_2(uint8_t reg, uint8_t value)
{
	...
	CSN_LOW_2();
	retval = hal_nrf_rw_2(W_REGISTER + reg);
	hal_nrf_rw_2(value);
	CSN_HIGH_2();
...
uint16_t hal_nrf_read_multibyte_reg_2(uint8_t reg, uint8_t *pbuf)
{
	...
	case HAL_NRF_PIPE1:
	case HAL_NRF_TX:
			length = ctr = hal_nrf_get_address_width_2();
			CSN_LOW_2();
			hal_nrf_rw_2(RX_ADDR_P0 + reg);
			break;

	case HAL_NRF_RX_PLOAD:
			if ( (reg = hal_nrf_get_rx_data_source_2()) < 7)
			{
				length = ctr = hal_nrf_read_rx_payload_width_2();

				CSN_LOW_2();
				hal_nrf_rw_2(R_RX_PAYLOAD);
			}
			else
			{
				ctr = length = 0;
...
/*******************************************************************************
  ...
  * @retval None
*******************************************************************************/
void hal_nrf_write_multibyte_reg_2(uint8_t cmd, const uint8_t *pbuf, uint8_t length)
{
	CSN_LOW_2();
	hal_nrf_rw_2(cmd);
	while(length--)
...
*******************************************************************************/
uint8_t hal_nrf_rw_2(uint8_t uWrittenByte)
{
	while(SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET);
	SPI_I2S_SendData(SPI2, uWrittenByte);
